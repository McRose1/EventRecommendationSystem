...
public class TicketMasterAPI {
	...
	private static final String API_KEY = "68cCYzYHWpsHqgoPG2IWadtdeOqcNlNQ";
		...
		if (!event.isNull("_embedded")) {
			...
			if (!embedded.isNull("venues")) {
			    ...
			    for (int i = 0; i < venues.length(); ++i) {
					...
					if (!venue.isNull(constants.address)) {		// 防止 typo，如果 address 拼错了，编译器会告诉我们
	...
	public List<Item> search(double lat, double lon, String keyword) {
		...
		try {
			...
			String inputLine;
			StringBuilder response = new StringBuilder();	// String is immutable
